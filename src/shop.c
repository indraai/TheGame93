...
const char *trade_letters[] = {
        ...
        "Evil",
        "Neutral",
        "Army",           /* Then the class based ones */
        "Navy",
        "Marines",
        "AirForce",
        "\n"
...
static int is_ok_char(struct char_data *keeper, struct char_data *ch, int shop_nr)
{
  ...
  if ((IS_ARMY(ch) && NOTRADE_MAGIC_USER(shop_nr)) ||
      (IS_NAVY(ch) && NOTRADE_MONK(shop_nr)) ||
      (IS_AIRFORCE(ch) && NOTRADE_THIEF(shop_nr)) ||
      (IS_MARINES(ch) && NOTRADE_WARRIOR(shop_nr))) {
...
static void shopping_buy(char *arg, struct char_data *ch, struct char_data *keeper, int shop_nr)
{
  ...
  if (OBJ_FLAGGED(obj, ITEM_QUEST)) {
    if (GET_OBJ_COST(obj) > GET_QUESTPOINTS(ch) && !IS_DEVA(ch)) {
  ...
  if (buy_price(obj, shop_nr, keeper, ch) > GET_GOLD(ch) && !IS_DEVA(ch)) {
    char actbuf[MAX_INPUT_LENGTH];
  ...
  if (OBJ_FLAGGED(obj, ITEM_QUEST)) {
    while (obj &&
           (GET_QUESTPOINTS(ch) >= GET_OBJ_COST(obj) || IS_DEVA(ch))
  ...
  while (obj && (GET_GOLD(ch) >= buy_price(obj, shop_nr, keeper, ch) || IS_DEVA(ch))
  ...
  if (!IS_DEVA(ch) && obj && !OBJ_FLAGGED(obj, ITEM_QUEST)) {
