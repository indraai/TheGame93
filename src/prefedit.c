...
static void prefedit_disp_toggles_menu(struct descriptor_data *d)
{
  ...
  send_to_char(d->character, "%s1%s) Autoexits    %s[%s%3s%s]      %sA%s) Gossip   %s[%s%3s%s]\r\n"
                             ...
                             "%s4%s) Autooffer    %s[%s%3s%s]      %sD%s) Auction  %s[%s%3s%s]\r\n"
...
/* Line 4 - AUTOOFFER and auction */
             CBYEL(d->character, C_NRM), CCNRM(d->character, C_NRM), CCCYN(d->character, C_NRM), PREFEDIT_FLAGGED(PRF_AUTOOFFER) ? CBGRN(d->character, C_NRM) : CBRED(d->character, C_NRM),
             ONOFF(PREFEDIT_FLAGGED(PRF_AUTOOFFER)), CCCYN(d->character, C_NRM), CBYEL(d->character, C_NRM), CCNRM(d->character, C_NRM), CCCYN(d->character, C_NRM),
...
void prefedit_parse(struct descriptor_data * d, char *arg)
{
  ...
  case PREFEDIT_TOGGLE_MENU:
    switch (*arg) {
      ...
      case '4':
        TOGGLE_BIT_AR(PREFEDIT_GET_FLAGS, PRF_AUTOOFFER);
        break;
...
void prefedit_Restore_Defaults(struct descriptor_data *d)
{
  ...
  /* PRF_AUTOOFFER    - Off */
  if (PREFEDIT_FLAGGED(PRF_AUTOOFFER))
     REMOVE_BIT_AR(PREFEDIT_GET_FLAGS, PRF_AUTOOFFER);
