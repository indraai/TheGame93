...
void free_message_list(struct message_list * mlist)
{
  ...
  while (msg) {
    ...
    free_messages_type(&msg->deva_msg);
...
void free_messages(void)
{
  ...
  for (i = 0; i < MAX_MESSAGES; i++)
    while (fight_messages[i].msg) {
      ...
      free_messages_type(&former->deva_msg);
...
void load_messages(void)
{
  ...
  while (fgets(chk, 128, fl)) {
    ...
    while (*chk == 'M') {
      ...
      messages->deva_msg.attacker_msg = fread_action(fl, i);
      messages->deva_msg.victim_msg = fread_action(fl, i);
      messages->deva_msg.room_msg = fread_action(fl, i);
    }
  }
  fclose(fl);
  ...
  for (i = 0; i < MAX_MESSAGES; i++) {
    ...
    for (msg = fight_messages[i].msg; msg; msg = msg->next){
      fprintf(fp, "M\n"
                  ...
                  PRINT_MSG(msg->deva_msg.attacker_msg),
                  PRINT_MSG(msg->deva_msg.victim_msg),
                  PRINT_MSG(msg->deva_msg.room_msg));
    }
  }
...
static void copy_message_strings(struct message_type *tmsg, struct message_type * fmsg)
{
  ...
  tmsg->deva_msg.attacker_msg = fmsg && fmsg->deva_msg.attacker_msg ? strdup(fmsg->deva_msg.attacker_msg) : NULL;
  tmsg->deva_msg.victim_msg = fmsg && fmsg->deva_msg.victim_msg ? strdup(fmsg->deva_msg.victim_msg) : NULL;
  tmsg->deva_msg.room_msg = fmsg && fmsg->deva_msg.room_msg ? strdup(fmsg->deva_msg.room_msg) : NULL;
}
...
void msgedit_parse(struct descriptor_data *d, char *arg)
{
  ...
  switch (OLC_MODE(d)) {
    case MSGEDIT_MAIN_MENU:
      ...
      switch (*arg) {
        ...
        case 'j':
          ...
          write_to_output(d, "Enter new string : ");
          OLC_MODE(d) = MSGEDIT_DEVA_CHAR;
        return;
        case 'K':
        case 'k':
          write_to_output(d, "Example: $n can't hit you!\r\n");
          write_to_output(d, "Enter new string : ");
          OLC_MODE(d) = MSGEDIT_DEVA_VICT;
        return;
        case 'L':
        case 'l':
          write_to_output(d, "Example: $n can't hit $N!\r\n");
          write_to_output(d, "Enter new string : ");
          OLC_MODE(d) = MSGEDIT_DEVA_ROOM;
        return;
        case 'N':
        case 'n':
    ...
    break;
    case MSGEDIT_DEVA_CHAR:
      if (!genolc_checkstring(d, arg))
        break;
      delete_doubledollar(arg);

      if (OLC_MSG(d)->deva_msg.attacker_msg)
        free(OLC_MSG(d)->deva_msg.attacker_msg);

      OLC_MSG(d)->deva_msg.attacker_msg = strdup(arg);
    break;
    case MSGEDIT_DEVA_VICT:
      if (!genolc_checkstring(d, arg))
        break;
      delete_doubledollar(arg);

      if (OLC_MSG(d)->deva_msg.victim_msg)
        free(OLC_MSG(d)->deva_msg.victim_msg);

      OLC_MSG(d)->deva_msg.victim_msg = strdup(arg);
    break;
    case MSGEDIT_DEVA_ROOM:
      if (!genolc_checkstring(d, arg))
        break;
      delete_doubledollar(arg);

      if (OLC_MSG(d)->deva_msg.room_msg)
        free(OLC_MSG(d)->deva_msg.room_msg);

      OLC_MSG(d)->deva_msg.room_msg = strdup(arg);
    break;
  }
