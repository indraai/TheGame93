...
ACMD(do_oasis_copy)
{
  ...
  if (src_rnum == NOWHERE) {
    send_to_char(ch, "\ninfo:The source %s (#%d) does not exist.\r", oasis_copy_info[i].text, src_vnum);
    return;
  }
  ...
  if (dst_rnum != NOWHERE) {
    send_to_char(ch, "\ninfo:The target %s (#%d) already exists.\r", oasis_copy_info[i].text, dst_vnum);
    return;
  }
  ...
  for (d = descriptor_list; d; d = d->next) {
	...
	send_to_char(ch, "\ninfo:The target %s (#%d) is currently being edited by %s.\r",
  ...
  if ((OLC_ZNUM(d) = real_zone_by_thing(dst_vnum)) == NOWHERE) {
    send_to_char(ch, "\nalert:Sorry, there is no zone for the given vnum #%d.\r", dst_vnum);
    free(d->olc);
    d->olc = NULL;
    return;
  }
  ...
  /* Perform the copy. */
  send_to_char(ch, "\ninfo: Copy %s from #%d to #%d.\r", oasis_copy_info[i].text, src_vnum, dst_vnum);
...
ACMD(do_dig)
{
  ...
  if (!*sdir || !*sroom) {
    send_to_char(ch, "\ncreate: dig [direction] [room] - to create an exit\r"
                     "\ncreate: dig [direction] -1     - to delete an exit\r");
    return;
  }
  ...
  if (W_EXIT(IN_ROOM(ch), dir)) {
      send_to_char(ch, "\ninfo:There already is an exit to the %s.\r", dirs[dir]);
      return;
  }
  ...
  if (zone == NOWHERE) {
    send_to_char(ch, "\ninfo:You cannot link to a non-existing zone!\r");
    return;
  }
  if (!can_edit_zone(ch, zone)) {
    send_to_char(ch, "\nalert:You do not have permission to edit room #%d.\r", rvnum);
    return;
  }
  ...
  if (rrnum == NOWHERE) {
    ...
    send_to_char(ch, "\ninfo:New room (%d) created.\r", rvnum);
    cleanup_olc(d, CLEANUP_ALL);
  ...
  send_to_char(ch, "\ninfo:You make an exit %s to room %d (%s).\r",
                   dirs[dir], rvnum, world[rrnum].name);

  /* Check if we can dig from there to here. */
  if (W_EXIT(rrnum, rev_dir[dir]))
    send_to_char(ch, "\ninfo:You cannot dig from %d to here. The target room already has an exit to the %s.\r",
...
int buildwalk(struct char_data *ch, int dir)
{
  ...
  if (!IS_NPC(ch) && PRF_FLAGGED(ch, PRF_BUILDWALK) &&
    ...
    if (!can_edit_zone(ch, world[IN_ROOM(ch)].zone)) {
      send_to_char(ch, "\nalert:You do not have build permissions in this zone.\r");
    } else if ((vnum = redit_find_new_vnum(world[IN_ROOM(ch)].zone)) == NOWHERE) {
      send_to_char(ch, "\nalert:No free vnums are available in this zone!\r");
    } else {
      ...
      sprintf(buf, "\ninfo:This unfinished room was created by %s.\r", GET_NAME(ch));
      ...
      send_to_char(ch, "\ninfo:%sRoom #%d created by BuildWalk.%s\r", yel, vnum, nrm);
