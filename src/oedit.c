...
static void oedit_disp_apply_menu(struct descriptor_data *d)
{
  ...
  for (i = 0; i < NUM_APPLIES; i++) {
    write_to_output(d, "\ncloud[%s]:%d\r", apply_types[i], ++count);
  }
  write_to_output(d, "\n::end:buttons\r"
    "\ncloud[done]:0\r");
...
static void oedit_disp_type_menu(struct descriptor_data *d)
{
  ...
  for (i = 0; i < NUM_ITEM_TYPES; i++) {
    write_to_output(d, "\ncloud[%s]:%d\r", item_types[i], count++);
  }
  write_to_output(d, "\n::end:buttons:\r"
    "\ncloud[done]:0\r");
}
...
static void oedit_disp_extra_menu(struct descriptor_data *d)
{
  ...
  for (i = 0; i < NUM_ITEM_FLAGS; i++) {
    write_to_output(d, "\ncloud[%s]:%d\r", extra_bits[i], ++count);
  }
...
static void oedit_disp_menu(struct descriptor_data *d)
{
  ...
  write_to_output(d,
    ...
    "\ncloud[applies to]:3\r"
    "\ncloud[copy object]:4\r"
    "\ncloud[delete object]:5\r"
    "\n::end:buttons\r"
	  "\ncloud[quit]:0\r",
	  buf1,                  //wear
    buf2,                 // affects
