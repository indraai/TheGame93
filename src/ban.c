...
ACMD(do_ban)
{
  ...
  mudlog(NRM, MAX(LVL_DEVA, GET_INVIS_LEV(ch)), TRUE, "%s has banned %s for %s players.",
...
ACMD(do_unban)
{
  ...
  mudlog(NRM, MAX(LVL_DEVA, GET_INVIS_LEV(ch)), TRUE, "%s removed the %s-player ban on %s.",
