...
char_data *get_char(char *name)
{
  ...
  if (*name == UID_CHAR) {
    ...
    if (i && valid_dg_target(i, DG_ALLOW_DEVAS))
      return i;
  } else {
    for (i = character_list; i; i = i->next)
      if (isname(name, i->player.name) &&
          valid_dg_target(i, DG_ALLOW_DEVAS))
        return i;
  }
 ...
 * only find deva characters if DG_ALLOW_DEVAS is on. */
char_data *get_char_near_obj(obj_data *obj, char *name)
{
  ...
  if (*name == UID_CHAR) {
    ...
    if (ch && valid_dg_target(ch, DG_ALLOW_DEVAS))
      return ch;
  } else {
    room_rnum num;
    if ((num = obj_room(obj)) != NOWHERE)
      for (ch = world[num].people; ch; ch = ch->next_in_room)
        if (isname(name, ch->player.name) &&
            valid_dg_target(ch, DG_ALLOW_DEVAS))
          return ch;
  }
...
char_data *get_char_by_obj(obj_data *obj, char *name)
{
  ...
  if (*name == UID_CHAR) {
    ...
    if (obj->carried_by &&
        ...
        valid_dg_target(obj->carried_by, DG_ALLOW_DEVAS))
      return obj->carried_by;

    if (obj->worn_by &&
        isname(name, obj->worn_by->player.name) &&
        valid_dg_target(obj->worn_by, DG_ALLOW_DEVAS))
      return obj->worn_by;
